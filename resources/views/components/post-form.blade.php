@props(['labelText', 'fieldName', 'placeholder', 'action'])
<form class="grow" method="POST" action="{{ $action }}">
    @csrf
    <label class="sr-only" for="post">{{ $labelText }}</label>
    <textarea class="w-full resize-none text-lg" name="{{ $fieldName }}" id="post"
