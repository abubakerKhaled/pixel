...
<div class="mt-6 h-full">
    <nav class="overflow-x-auto [scrollbar-width:none]">
        <ul class="flex min-w-max justify-end gap-8 text-sm">
            <li>
                <a href="{{ route('profiles.show', $profile) }}" @class(['text-pixl-light/60 hover:text-pixl-light/80' => $activeTab !== 'posts'])>Posts</a>
            </li>
            <li>
                <a href="{{ route('profiles.replies', $profile) }}" @class(['text-pixl-light/60 hover:text-pixl-light/80' => $activeTab !== 'replies'])>Replies</a>
            </li>
            <li>
